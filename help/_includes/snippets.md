...
>[!IMPORTANT] When selecting the Environment field in the new ticket form, use Adobe's environment naming (for example, select Staging even if you call that environment **Dev** internally). You may mention your internal name in the description, but the Environment field itself must use Adobe's nomenclature.
