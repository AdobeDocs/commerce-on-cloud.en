...
The [!DNL Adobe Commerce Traffic Insights] app visualizes raw Fastly Content Delivery Network (CDN) access logs into a picture of a store's edge traffic. Charts are grouped into the following tabs:

- **Bandwidth** — How traffic bandwidth is distributed across domains, content and resource types, and Cloud projects over time.
- **Full Page Cache Performance** — How efficiently dynamic storefront HTML for product detail pages (PDP), product listing pages (PLP), and content management system (CMS) pages is cached at the edge.
- **Bots Activity and Requests Analysis** — Traffic broken down by known bot agents, geolocation, IPs/subnets, URLs, and Fastly Next-Gen Web Application Firewall (WAF) signals.
...
- **Developers** tuning Full Page Cache (FPC) coverage and hit ratio, or implementing Fastly Varnish Configuration Language (VCL) rules.
...
- **Requests (Req.)** is the number of distinct requests, however, with Fastly [shielding](https://www.fastly.com/documentation/guides/concepts/shielding/) enabled a single request is logged **twice**, once on each of the following:
  - Internal shield [Point of Presence (POP)](https://www.fastly.com/documentation/guides/getting-started/concepts/using-fastlys-global-pop-network/) 
  - Edge POP 
